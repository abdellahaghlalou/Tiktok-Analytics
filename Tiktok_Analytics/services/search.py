...
class Search:
    ...
    def __init__(self,option : int,search_word : str,search_result: Optional[List[Union[User,Video]]]) -> None:
        self.option = option
        self.searcrh_word = search_word
        self.search_result = search_result
    
    def search(self,browser:Browser) -> None:
        if self.option == 1:
            self.search_result = self.search_by_user(self.search_word,browser)
        if self.option == 2:
            self.search_result = self.search_by_video(self.search_word,browser)
        if self.option == 3:
            self.search_result = self.search_by_hashtag(self.search_word,browser)
        if self.option == 4:
            self.search_result = self.search_by_sound(self.search_word,browser)

    def search_by_user(self,search_word : str,browser:Browser) -> List[User]:

        browser = browser.lunch_browser()
